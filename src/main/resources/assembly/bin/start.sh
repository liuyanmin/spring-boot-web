...
#后台启动，打印控制台日志
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.lym.springboot.web.Application > $STDOUT_FILE 2>&1 &
#后台启动，不打印控制台日志
#nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.lym.springboot.web.Application > /dev/null 2>&1 &
